...
export default function DivisionScreen({ navigation }) {
  // State variables
  ...
  const generateProblem = () => {
    let range1 = calculateRange(digits1);
    let range2 = calculateRange(digits2);
    
    // Ensure numerator will be larger than denominator
    if (range1 < range2) {
      [range1, range2] = [range2, range1]; // Swap ranges
    }
    
    const min2 = range2 / 10;
    const denominator = Math.floor(Math.random() * (range2 - min2)) + min2;
    
    // Find maximum possible multiplier based on numerator range
    const maxPossibleMultiplier = Math.floor(range1 / denominator);
    const multiplier = Math.floor(Math.random() * maxPossibleMultiplier) + 1;
    
    const numerator = denominator * multiplier;
    
    setNumber1(numerator);
    setNumber2(denominator);
    setUserAnswer('0');
    setFeedback('');
    setSolutionText('');
  ...
  const checkAnswer = () => {
    ...
    const userAns = parseFloat(userAnswer) || 0;
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.headerText}>Arithmetic Trainer: Division</Text>
      </View>
      ...
      <View style={styles.mainContent}>
        <View style={styles.mathLayout}>
          ...
          <Text style={styles.noteText}>
            *The numerator will be the larger number between num1 and num2
          </Text>
        ...
        <View style={styles.settings}>
          ...
          <Text style={styles.settingLabel}>Digit length (1-7):</Text>
          
          <View style={styles.settingGroup}>
            <Text style={styles.settingText}>Num1</Text>
            <TextInput
          ...
          <View style={styles.settingGroup}>
            <Text style={styles.settingText}>Num2</Text>
            <TextInput
              style={styles.digitInput}
